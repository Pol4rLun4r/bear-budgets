# Database

Definições e explicações do banco de dados e regras de negócio de suas tabelas.

## Tabelas

Explicações das tabelas, campos e regras de negócios da mesma.

> [!Note]
> `Obrigatório` seria: `NOT NULL` em `sqlite`

 - [quotations](quotations) -> dados da cotação
 - [item_references](item_references)  -> dados básicos do item
 - [item_values](item_values) -> valores do item
 - [reference_links](reference_links) -> links de referencia do item 
 - [quotation_links](quotation_links) -> conexão entre cotação, item e valores do item