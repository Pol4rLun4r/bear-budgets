...
describe("POST /quotations/items - Create Quotation and Add Items - Part 1 success create client", () => {
    ...
    it("ter sucesso ao usar um cliente que já existe", async () => {
        ...
        // 1. prepara o payload
        const client = {
            name: "Games Acer LTDA",
            document: "25.004.056/0001-45",
            notes: "novo cliente",
            type_client: "nacional"
        }

        const clientId = repo.client.create({ ...client } as any)?.id;
        ...
        for (let index = 0; index < responseLink.length; index++) {
            ...
            // 3.1 verifica se o nome do cliente é o mesmo informado
            expect(resClientData?.name).toBe(onlyName(client.name));

            // 3.2 verifica se o documento do cliente é o mesmo informado
            expect(resClientData?.document).toBe(normalizeDocument(client.document));

            // 3.3 verifica se as notas do cliente é a mesma informada
            expect(resClientData?.notes).toBe(client.notes);

            // 3.4 verifica se o typo de cliente é o mesmo informado
            expect(resClientData?.type_client).toBe(client.type_client);
        }
    });
    ...
    it("ter sucesso ao criar um cliente que já existe", async () => {
        ...
        const client = {
            ...
            document: "25.004.056/0001-45",
            notes: "novo cliente",
            type_client: "nacional" as ClientType
        }

        repo.client.create({ ...client })?.id;
